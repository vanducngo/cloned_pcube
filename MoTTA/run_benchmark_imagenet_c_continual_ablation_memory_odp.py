...
def main_continual_ablation():
    device = torch.device("cuda")
    
    MODES = [
        "Source_Only",
        "MoTTA_Original",
        "Ablation_ODP_Block_Mem_MoTTA",
        "Ablation_ODP_Orig_Mem_AAMP",
        "MoTTA_AAMP"
    ]
    
    # Mở file CSV để ghi kết quả
    with open("continual_ablation_results.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Mode"] + CORRUPTIONS + ["Final_Avg_Error"])

    for mode in MODES:
        print(f"\n[Starting Continual Run] Mode: {mode}")
        wandb.init(project="MoTTA_Continual_Ablation", name=mode, reinit=True)
        ...
        # Cấu hình AAMP từ mode
        if mode != "Source_Only" and mode != "MoTTA_Original":
            if not hasattr(cfg, 'paras_adapt_model'): cfg.paras_adapt_model = cdict()
            if mode == "MoTTA_AAMP":
                cfg.paras_adapt_model.ablation_odp_type = "blockwise"
                cfg.paras_adapt_model.ablation_memory_type = "aamp"
            elif mode == "Ablation_ODP_Block_Mem_MoTTA":
        ...
        # 2. VÒNG LẶP LIÊN TỤC (KHÔNG RESET)
        results_row = []
        ...
        for corruption in CORRUPTIONS:
            ...
            # Log kết quả từng corrupt trên wandb
            wandb.log({f"error_rate/{corruption}": error_rate})
            print(f"  {corruption}: {error_rate:.2f}%")
        
        # Kết quả cuối
        avg_err = 100 - (total_correct / total_all) * 100
        results_row.append(f"{avg_err:.2f}")
        
        with open("continual_ablation_results.csv", "a", newline="") as f:
            writer = csv.writer(f)
