...
class MoTTA_AAMP(nn.Module):
    def __init__(self, model, paras_optim, capacity, num_classes, bn_alpha, temp_factor,
                 ...
                 ablation_odp_type = "blockwise", ablation_memory_type = "aamp",
                 ablation_use_adaptive_aging = False,
                 ablation_lambda_t = 1.0,
                 ablation_lambda_u = 1.0,
                 ablation_lambda_odp = 1.0
                 ):
        ...
        p_cube_cfg = ModuleConfig(
            ...
            ablation_use_adaptive_aging=ablation_use_adaptive_aging,
            lambda_t=ablation_lambda_t,
            lambda_u=ablation_lambda_u,
            lambda_odp=ablation_lambda_odp
        )
    ...
    def forward(self, x):
        ...
        # Đếm theo số mẫu trong batch (x.shape[0]) thay vì +1
        self.num_instance += x.shape[0]
        if self.num_instance >= self.update_frequency:
            update_model_flag = True

        # update model
        if update_model_flag:
            # [CHANGE]
            sup_data, sup_weights = self.p_cube.memory.get_memory()
            if sup_data is not None and len(sup_data) > 0:
                if isinstance(sup_data, list):
                    sup_data = torch.stack(sup_data)
                self.update_model(sup_data)
